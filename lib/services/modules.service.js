...
function sanitizeZipPayload(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new ValidationError('moduleSpecsZip must be an object');
  }

  const name = String(value.name || '').trim();
  const dataUrl = String(value.dataUrl || '').trim();
  if (!name || !dataUrl) {
    throw new ValidationError('moduleSpecsZip.name and moduleSpecsZip.dataUrl are required');
  }
...
function readRawLayout(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return {};
  return { ...value };
}

function buildChunkSession(payload) {
  const uploadId = String(payload?.uploadId || '').trim();
  const fileName = String(payload?.fileName || '').trim();
  const fileType = String(payload?.fileType || 'application/zip').trim() || 'application/zip';
  const uploadedAt = String(payload?.uploadedAt || '').trim() || new Date().toISOString();
  const fileSize = toPositiveInt(payload?.fileSize, 0);
  const chunkIndex = toPositiveInt(payload?.chunkIndex, -1);
  const totalChunks = toPositiveInt(payload?.totalChunks, 0);
  const chunkData = String(payload?.chunkData || '');

  if (!uploadId) throw new ValidationError('uploadId is required');
  if (!fileName) throw new ValidationError('fileName is required');
  if (chunkIndex < 0) throw new ValidationError('chunkIndex must be a non-negative integer');
  if (totalChunks <= 0) throw new ValidationError('totalChunks must be greater than 0');
  if (chunkIndex >= totalChunks) throw new ValidationError('chunkIndex must be less than totalChunks');
  if (!chunkData) throw new ValidationError('chunkData is required');
...
function upsertUploadChunk(rawLayout, chunkPayload) {
  const current = (rawLayout.zip_upload && typeof rawLayout.zip_upload === 'object' && !Array.isArray(rawLayout.zip_upload))
    ? rawLayout.zip_upload
    : null;

  const sameSession = current
    && current.uploadId === chunkPayload.uploadId
    && Number(current.totalChunks) === Number(chunkPayload.totalChunks);

  const chunks = sameSession && Array.isArray(current.chunks)
    ? [...current.chunks]
    : new Array(chunkPayload.totalChunks).fill('');

  chunks[chunkPayload.chunkIndex] = chunkPayload.chunkData;
  const receivedChunks = chunks.reduce((count, part) => count + (part ? 1 : 0), 0);
  const complete = receivedChunks === chunkPayload.totalChunks;
  ...
  return {
    nextRawLayout: {
      ...
      zip_upload: {
        ...
        fileSize: chunkPayload.fileSize,
        uploadedAt: chunkPayload.uploadedAt,
...
function finalizeChunkUpload(rawLayout) {
  const session = rawLayout?.zip_upload;
  if (!session || typeof session !== 'object' || Array.isArray(session)) {
    throw new ValidationError('Invalid zip upload session');
  }

  const chunks = Array.isArray(session.chunks) ? session.chunks : [];
  const totalChunks = toPositiveInt(session.totalChunks, 0);
  if (totalChunks <= 0 || chunks.length < totalChunks) {
    throw new ValidationError('Zip upload is incomplete');
  }
  ...
  return {
    name: String(session.fileName || '').trim(),
    type: String(session.fileType || 'application/zip').trim() || 'application/zip',
    size: toPositiveInt(session.fileSize, 0),
    uploadedAt: String(session.uploadedAt || '').trim() || new Date().toISOString(),
    dataUrl: `data:${String(session.fileType || 'application/zip').trim() || 'application/zip'};base64,${base64Data}`,
  };
}

function mergeLayoutPreserveZip(existingLayout, incomingLayout) {
  const existing = normalizeSpecLayout(existingLayout);
  const incoming = normalizeSpecLayout(incomingLayout);
  if (!incoming.moduleSpecsZip && existing.moduleSpecsZip) {
    incoming.moduleSpecsZip = existing.moduleSpecsZip;
  }
  return incoming;
}
...
export async function updateModuleSpecifications(id, payload, actor) {
  ...
  const nextLayout = mergeLayoutPreserveZip(mod.spec_layout, payload.layout);
...
export async function updateTeamsModuleSpecifications(id, payload, actor) {
  ...
  const nextLayout = mergeLayoutPreserveZip(mod.team_spec_layout, payload.layout);
...
export async function updateModuleSpecificationsZip(id, payload, actor) {
  const mod = await requireModule(id, actor);
  if (payload?.moduleSpecsZip) {
    const moduleSpecsZip = sanitizeZipPayload(payload?.moduleSpecsZip);
    const nextLayout = normalizeSpecLayout(mod.spec_layout);
    nextLayout.moduleSpecsZip = moduleSpecsZip;

    const updated = await prisma.module.update({
      where: { id: mod.id },
      data: { spec_layout: nextLayout },
    });

    const layout = updated.spec_layout && typeof updated.spec_layout === 'object' && !Array.isArray(updated.spec_layout)
      ? normalizeSpecLayout(updated.spec_layout)
      : normalizeSpecLayout(null);

    return { layout, completed: true };
  }

  const chunk = buildChunkSession(payload);
  const rawLayout = readRawLayout(mod.spec_layout);
  const { nextRawLayout, receivedChunks, complete } = upsertUploadChunk(rawLayout, chunk);

  if (!complete) {
    await prisma.module.update({
      where: { id: mod.id },
      data: { spec_layout: nextRawLayout },
    });
    return { completed: false, receivedChunks, totalChunks: chunk.totalChunks };
  }

  const moduleSpecsZip = finalizeChunkUpload(nextRawLayout);
  const finalizedLayout = normalizeSpecLayout(nextRawLayout);
  finalizedLayout.moduleSpecsZip = moduleSpecsZip;

  await prisma.module.update({
    where: { id: mod.id },
    data: { spec_layout: finalizedLayout },
  });

  return { layout: finalizedLayout, completed: true, receivedChunks: chunk.totalChunks, totalChunks: chunk.totalChunks };
}

export async function updateTeamsModuleSpecificationsZip(id, payload, actor) {
  const mod = await requireTeamsModule(id, actor);
  if (payload?.moduleSpecsZip) {
    const moduleSpecsZip = sanitizeZipPayload(payload?.moduleSpecsZip);
    const nextLayout = normalizeSpecLayout(mod.team_spec_layout);
    nextLayout.moduleSpecsZip = moduleSpecsZip;

    const updated = await prisma.module.update({
      where: { id: mod.id },
      data: { team_spec_layout: nextLayout },
    });

    const layout = updated.team_spec_layout && typeof updated.team_spec_layout === 'object' && !Array.isArray(updated.team_spec_layout)
      ? normalizeSpecLayout(updated.team_spec_layout)
      : normalizeSpecLayout(null);
  ...
  const chunk = buildChunkSession(payload);
  const rawLayout = readRawLayout(mod.team_spec_layout);
  const { nextRawLayout, receivedChunks, complete } = upsertUploadChunk(rawLayout, chunk);

  if (!complete) {
    await prisma.module.update({
      where: { id: mod.id },
      data: { team_spec_layout: nextRawLayout },
    });
    return { completed: false, receivedChunks, totalChunks: chunk.totalChunks };
  }
  ...
  await prisma.module.update({
    where: { id: mod.id },
    data: { team_spec_layout: finalizedLayout },
  });
