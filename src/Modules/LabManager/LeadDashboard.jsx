...
function FloatingSaveButton({ onSave, saveStatus }) {
...
export default function LeadDashboard({ moduleId, user, onBack }) {
  ...
  const [pairSaveStatus, setPairSaveStatus] = useState('idle');
    ...
    setModuleData(prev => {
      ...
      return {
        ...
        groups: (prev.groups || []).map(g =>
          g.id === group.id ? { ...g, domain_leads: items } : g
        ),
      };
    });
    try {
      await updateGroup(moduleId, group.id, { domain_leads: items });
    } catch (err) {
      showToast('error', err.message || 'Failed to update domain leads.');
      await loadData({ silent: true }); // rollback on error
    }
  };
  ...
  const handleUpdateRollNumbers = async (group, items) => {
    ...
    setModuleData(prev => {
      ...
      return {
        ...prev,
        groups: (prev.groups || []).map(g =>
          g.id === group.id ? { ...g, roll_numbers: items } : g
        ),
      };
    });
    try {
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-gray-50">
      ...
      {/* ── Floating save button (evaluation view only) ── */}
      {primaryView === 'evaluation' && selectedGroup && (
        <FloatingSaveButton onSave={handleManualSave} saveStatus={pairSaveStatus} />
      )}
